...
const cities = [
  ...
  //sub { name: 'San Jose', lat: 37.3473, long: -121.8671 },
  { name: 'San Francisco', lat: 37.8079, long: -122.4191 },
  ...
  { name: 'Jacksonville', lat: 30.3446, long: -81.6630 },
  ...
  { name: 'Berlin', lat: 52.5209, long: 13.3894 },
  ...
  { name: 'Budapest', lat: 47.5213, long: 19.05646 },
  { name: 'Debrecen', lat: 47.5415, long: 21.6216 },
...
cities.push(
  ...
  { name: 'Lima', lat: -12.0887, long: -77.0476 },
...
cities.push(
  ...
  { name: 'Hanoi', lat: 21.0326, long: 105.8521 },
...
rl.on('line', async (input) => {
  ...
  if (command.startsWith('retry ')) {
    const cityName = raw.slice(6).trim();
    const bboxOffset = 0.003;

    if (!cityName || Number.isNaN(bboxOffset)) {
      console.log("Usage: retry <cityName>");
      return;
    }
    try {
      ...
      console.error(`Failed to retry cache fill for ${cityName}:`, err.message || err);
      return;
    }
  ...
  if(command.startsWith("check ")){
    ...
    if(arg.toLowerCase() === "all"){
      for(i = 0; i < cities.length; i++){
        ...
        if(!imageIds){
          ...
          console.log(" returned something undefined!");
          failCnt++;
          ...
          console.log(city.name);
          console.log(" Had 0 image ids")
          failCnt++;
        } else if(imageIds.length < 20){
          console.log(city.name);
          console.log(" Had " + imageIds.length + " image ids");
    ...
    if (cityIndex === -1) {
      console.log(`Could not find city to: ${arg}`);
      return;
    }
