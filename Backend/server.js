...
rl.on('line', async (input) => {
  ...
  if (command.startsWith('add ')) {
    ...
    try {
      ...
      if (!city || typeof city.name !== 'string') throw new Error('city.name must be a string');
      const lat = Number(city.lat);
      const lon = Number(city.long ?? city.lng ?? city.lon ?? city.longitude);
      if (Number.isNaN(lat) || Number.isNaN(lon)) throw new Error('lat and long must be numbers');
